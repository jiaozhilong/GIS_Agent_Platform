...
const route = useRoute(); const bases = ref<KnowledgeBase[]>([]); const result = ref<RetrievalResult | null>(null); const loading = ref(false); const error = ref('')
const query = ref('')
const selected = ref<string[]>([])
const topK = ref(5); const threshold = ref(.2)
const canRun = computed(() => query.value.trim().length > 8 && selected.value.length > 0)
const run = async () => { if(!canRun.value)return;loading.value=true;error.value='';try{result.value=await api.retrieve(String(route.params.id),{query:query.value,knowledgeBaseIds:selected.value,topK:topK.value,similarityThreshold:threshold.value})}catch(e){error.value=e instanceof Error?e.message:'知识检索失败'}finally{loading.value=false} }
onMounted(async()=>{try{const [knowledge,project]=await Promise.all([api.knowledgeBases(),api.project(String(route.params.id))]);bases.value=knowledge;selected.value=project.knowledgeBaseIds.filter(id=>knowledge.some(base=>base.id===id));if(!selected.value.length)selected.value=knowledge.filter(base=>base.chunkCount>0).map(base=>base.id);query.value=`围绕“${project.name}”的需求，检索总体架构、数据治理、GIS服务、产品能力和实施建议。原始需求：${project.rawDemand}`;}catch(e){error.value=e instanceof Error?e.message:'检索页面加载失败'}})
</script>
<template>
...
.page-error{padding:9px 12px;color:var(--danger);border:1px solid rgba(255,90,110,.25)}
</style>
