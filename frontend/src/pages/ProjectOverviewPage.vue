...
const stageLabels = { DRAFT:'项目初始化', REQUIREMENT_ANALYSIS:'需求分析', PRODUCT_MATCH:'产品匹配', KNOWLEDGE_RETRIEVAL:'知识检索', PROPOSAL_GENERATION:'方案生成', REVIEW:'方案评审', DELIVERED:'交付完成' } as const
const currentStage = computed(() => project.value ? stageLabels[project.value.stage] : '加载中')
const steps = ['需求分析','产品匹配','知识检索','方案生成','交付验收']
const completedSteps = computed(() => Math.floor((project.value?.progress || 0) / 20))
const toggleLayers = () => {
  ...
  <AppShell :title="project?.name || '项目作战室'" :subtitle="project ? `${project.customerName} · ${project.projectCode}` : '正在加载项目'">
    ...
    <div class="war-room">
      <aside class="project-info tech-panel">
        ...
        <dl><dt>项目名称</dt><dd>{{ project?.name }}</dd><dt>客户单位</dt><dd>{{ project?.customerName }}</dd><dt>所属行业</dt><dd>{{ project?.industry }}</dd><dt>项目负责人</dt><dd>{{ project?.ownerName }}</dd><dt>交付日期</dt><dd>{{ project?.deliveryDeadline }}</dd><dt>当前阶段</dt><dd class="score">{{ currentStage }}</dd></dl>
        <div class="project-progress"><div class="ring" :style="{'--progress':`${project?.progress ?? 0}%`}"><strong>{{ project?.progress ?? 0 }}%</strong></div><div><b>项目进度</b><span v-for="(step,index) in steps" :key="step"><i :class="{ done: index < completedSteps }" />{{ step }}</span></div></div>
      </aside>
