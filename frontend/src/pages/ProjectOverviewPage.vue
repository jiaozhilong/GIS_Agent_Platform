...
<template>
  <AppShell :title="project?.name || '项目作战室'" :subtitle="project ? `${project.customerName} · ${project.projectCode}` : '正在加载项目'">
    ...
    <div class="war-room">
      ...
      <section class="scene tech-panel">
        <div class="scene-tools"><div class="mode"><button :class="{ active: sceneMode === '2D' }" @click="sceneMode = '2D'">2D</button><button :class="{ active: sceneMode === '3D' }" @click="sceneMode = '3D'">3D</button></div><button><IconLayersIntersect :size="16" /></button><button><IconMap2 :size="16" /></button><button><IconArrowsMaximize :size="16" /></button></div>
        <CesiumProjectScene v-if="sceneMode === '3D'" />
