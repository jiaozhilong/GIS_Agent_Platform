...
<template>
  <AppShell title="方案生成" subtitle="RAGFlow 知识检索与 DeepSeek 联合生成，结果保留证据覆盖率">
    ...
    <div class="proposal-layout">
      <aside class="outline tech-panel"><div class="panel-title"><span>方案章节</span><span class="score">{{run?.sections.length || 0}}</span></div><button v-for="item in run?.sections" :key="item.id" :class="{active:activeSection===item.id}" @click="activeSection=item.id;view='outline'"><span>{{item.title}}</span><i>{{item.sourceType}}</i></button><div v-if="!run?.sections.length" class="empty">点击“开始生成”，系统会先检索知识库，再由 DeepSeek 补全方案。</div></aside>
      <section class="slide tech-panel">
        ...
        <div v-else class="document-editor"><h2>{{section?.title || '尚未生成正文'}}</h2><div v-if="loading&&!section" class="generating"><IconSparkles :size="26"/>RAGFlow 正在检索知识库并调用 DeepSeek，请稍候...</div><textarea v-else class="tech-textarea" :value="section?.content || '点击上方“开始生成”运行真实联合方案流程。'" readonly/><div v-if="section" class="citation-note">内容来源：{{section.sourceType}}　证据覆盖率：{{Math.round(section.evidenceCoverage*100)}}%　引用：{{section.citationIds.length}} 条</div></div>
