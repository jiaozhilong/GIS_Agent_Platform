...
const route = useRoute(); const loading = ref(false); const result = ref<RequirementAnalysis | null>(null); const error = ref('')
const rawDemand = ref('')
const run = async () => { loading.value = true; error.value = ''; try { result.value = await api.analyzeRequirements(String(route.params.id)) } catch (e) { error.value = e instanceof Error ? e.message : '需求分析失败' } finally { loading.value = false } }
onMounted(async () => { try { rawDemand.value = (await api.project(String(route.params.id))).rawDemand } catch (e) { error.value = e instanceof Error ? e.message : '项目需求加载失败' } })
...
<template>
  <AppShell title="需求分析" subtitle="AI 提取需求要点、能力维度与产品方向">
    ...
    <div class="analysis-grid">
      <section class="source tech-panel"><div class="panel-title"><span>客户原始需求</span><button class="ghost-button" @click="run"><IconRefresh :size="15" />重新分析</button></div><textarea v-model="rawDemand" class="tech-textarea" readonly /><button class="primary-button" :disabled="loading" @click="run"><IconPlayerPlay :size="16" />{{ loading ? 'Agent分析中...' : '运行需求分析Agent' }}</button><div class="demand-points"><h3>需求要点</h3><ul><li v-for="point in result?.demandPoints" :key="point">{{ point }}</li></ul></div><div class="case"><span>推荐行业案例</span><b>XX市国土空间基础信息平台</b><small>自然资源 · 匹配度 96%</small></div></section>
